...
export function ProductDetailClient({
  ...
  // Auto-select the first in-stock option (or just the first option, if none
  // are marked in stock) per variant group, so a product with variants is
  // immediately purchasable without forcing an extra click — and so the
  // buttons visually show a selection right away instead of looking inert.
  const [selected, setSelected] = useState<Record<string, string>>(() => {
    const initial: Record<string, string> = {};
    for (const group of product.variantGroups) {
      const inStock = group.options.find((o) => o.stock == null || o.stock > 0);
      const fallback = group.options[0];
      const pick = inStock ?? fallback;
      if (pick) initial[group.id] = pick.id;
    }
    return initial;
  });
