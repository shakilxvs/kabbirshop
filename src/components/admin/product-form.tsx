...
export async function ProductForm({ product }: { product?: Product }) {
  ...
  const variantsText = (product?.variantGroups ?? [])
  ...
  return (
    <form action={saveProduct} className="space-y-8 max-w-3xl">
      ...
      <Section title="Variants">
        <Field label={`Variant Groups — one per line: "Color: Black, White, Blue"`}>
          <textarea name="variants" defaultValue={variantsText} rows={3} className="input" placeholder="Color: Black, White, Blue" />
        </Field>
      </Section>
